...
\begin{equation*}
    \mathrm{d}\ln(K_t) \equiv \sum_{i=1}^N b_{it}^K \mathrm{d}\ln(K_{it}) \quad \text{where} \quad b_{it}^K \equiv \frac{r_{it} K_{it}}{\sum_{j=1}^N r_{jt} K_{jt}}
\end{equation*}
and the growth rate of aggregate labor is given by:
\begin{equation*}
    \mathrm{d}\ln(L_t) \equiv \sum_{i=1}^N b_{it}^L \mathrm{d}\ln(L_{it}) \quad \text{where} \quad b_{it}^L \equiv \frac{w_{it} L_{it}}{\sum_{j=1}^N w_{jt} L_{jt}}
\end{equation*}
...
\begin{align*}
    ...
    &+ \sum_{i=1}^N (b_{it} \alpha_{it}^K - b_{it}^K \lambda_t^K) \mathrm{d}\ln(K_{it}) \\
    &+ \sum_{i=1}^N (b_{it} \alpha_{it}^L - b_{it}^L \lambda_t^L) \mathrm{d}\ln(L_{it}).
\end{align*}
...
\begin{alignat}{2}
    ...
    &+ \sum_{t=t_0}^{t_1} \sum_{i=1}^N (b_{it} \alpha_{it}^K - b_{it}^K \lambda_t^K) \mathrm{d}\ln(K_{it}) &&\quad \text{\textcolor{HEC}{Capital}} \label{eq:capital} \\
    &+ \sum_{t=t_0}^{t_1} \sum_{i=1}^N (b_{it} \alpha_{it}^L - b_{it}^L \lambda_t^L) \mathrm{d}\ln(L_{it}) &&\quad \text{\textcolor{HEC}{Labor.}} \label{eq:labor}
\end{alignat}
